...
export const ComplaintCard: React.FC<ComplaintCardProps> = ({
  ...
  return (
    <div className={`relative bg-white rounded-2xl p-4 md:p-5 border transition-all duration-200 shadow-sm hover:shadow-md ${
      status === 'OVERDUE' 
      ...
      <p 
        ...
        className="text-slate-600 text-xs md:text-sm line-clamp-2 mb-4 bg-slate-50 p-2.5 rounded-xl border border-slate-100 cursor-pointer hover:bg-slate-100 transition-colors"
      >
        {complaint.description}
      </p>

      {/* Dates & Timeline grid */}
      <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 mb-4 bg-white p-2 rounded-xl border border-slate-100">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span>Sent to Sup: <strong className="text-slate-700">{formatDateSafe(complaint.dateSentToSupplier)}</strong></span>
        </div>
        {complaint.dateResponseReceived && (
          <div className="col-span-2 flex items-center gap-1.5 text-emerald-700 pt-1 border-t border-slate-100">
            ...
            <span>Response Received: <strong>{formatDateSafe(complaint.dateResponseReceived)}</strong></span>
          </div>
        )}
      </div>
